        ...
        CoroutineScope(Main).launch {
            ...
            if (campaigns != null) {
               /* val noOfRowDeleted = */dbDeleteTriggerEvents(context)
                ...
                logger.info("Notification fetch failed.")
        }

    }

    private suspend fun requestTriggerEventsFromCloud(context: Context): List<CampaignModel>? {
        if (!InAppNotification.hasInternet) {
            logger.error("Error: No Internet.")
            return null
        }

        if (InAppNotification.userId.isBlank()) {
        ...
        return withContext(IO) {
                ...
                .makeNotificationQuery(InAppNotification.apiKey, InAppNotification.userId)
//            showApiLog(eventsResponse)
            if (eventsResponse.isSuccessful && eventsResponse.body() != null) {
                ...
                withContext(Main) {
                    logger.debug("requestTriggerEventsFromCloud:  Error while getting data.")
                }
                null
            }
        }
...
//            eventBody.addProperty("tz", TimeZone.getDefault().displayName)
            ...
            val response = ServiceGenerator.requestApi()
                .logEventView(InAppNotification.apiKey, logCampaign.jsonObject)
        ...
        logger.debug("$context, $eventBody, $logCampaign")
        }
        return ""
    }
    ...
    private suspend fun updateTriggerEventLogToCloudWithCount(context: Context, eventBody: JsonObject, tryCount: Int): String {
        if (!InAppNotification.hasInternet) {
            logger.debug("Error: No Internet.")
...
//        }
    ...
    fun observeDatabaseNotification(context: Context, viewLifecycleOwner: LifecycleOwner) {
        CoroutineScope(Default).launch {
            ...
            if (notificationObserver?.hasObservers() != null){
                val db = CampaignDatabaseHelperImpl(DatabaseBuilder.getInstance(context))
                logger.debug("hasActiveObservers: ${db.getLiveDataCampaignsFromDb().hasActiveObservers()}")
                logger.debug("hasObservers: ${db.getLiveDataCampaignsFromDb().hasObservers()}")
            }
            ...
            if (notificationObserver?.hasObservers() == null || !notificationObserver!!.hasObservers()) {
                withContext(Main) {
                    ...
                    db.getLiveDataCampaignsFromDb().observe(viewLifecycleOwner) { it ->
                        logger.debug("observeDatabaseNotification: ${it?.size} notifications added/replaced.")
                        it.forEach {
                            logger.debug("observeDatabaseNotification: ${it.notificationId}")
                        }
                    }
                }
            }
    ...
    private suspend fun evaluateDbTriggerEvent(
        ...
        withContext(Default) {
            logger.debug("**** evaluateDbTriggerEvent:: ****\teventParam:${eventParam.toList()}")
...
//                        && triggerEventModel.minIntervalBtwDisplays >= (timeRightNow - triggerEventModel.lastDisplayedTime)
                    ...
                    /*if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        ...
                        logger.debug("timeRightNow: ${timeRightNow.toEpochMilli()}, Event endTime: ${triggerEventModel.endTs}, startTime: ${triggerEventModel.startTs}")
    ...
    internal fun findAndLaunchDbTriggerEvent(context: Context) = CoroutineScope(Default).launch {
        ...
        logger.debug("findAndLaunchTriggerNotification: ${dbTriggerEvents.map { it.notificationId }}")
    ...
    CoroutineScope(Main).launch {
        if (eventType == 0 || eventType == 1 || eventType == 2) {
            ...
            withContext(Default) {
                ...
                logger.debug("findAndLaunchTriggerNotification: ${dbTriggerNotifications.map { it.notificationId }}")
...
//                                Toast.makeText(context, "No ${triggerEventType.name} event found in the database.", Toast.LENGTH_SHORT).show()
                                logger.debug("findAndLaunchTriggerEventForTest: No ${triggerEventType.name} event found in the database.")
                            }
                        }
                    }
                }
    ...
    private fun launchModalInApp(context: Context, campaignModel: CampaignModel) {
        ...
        if (buttons.size() < 2) {
            logger.debug("launchModalTriggerNotification: Event is not valid for notificationId ${campaignModel.notificationId}.")
            return
        }
...
//                        else -> {
//                            logger.error("Unknown button error")
//                        }
                    }
    ...
    private fun prepareEventButtonClickActionBodyData(jsonObjectBody: JsonObject, jsonButton: JsonObject):JsonObject {
        ...
        logger.debug("Event Action API Body: $jsonObjectBody")
    ...
    private fun showApiLog(cloudEventResponse: Response<List<CampaignModelApi>>) {
        logger.debug("************* fetchCloudEvents FETCHED *************\n")
        logger.debug("1. isSuccessful: ${cloudEventResponse.isSuccessful}")
//        logger.debug("2. Body: ${cloudEventResponse.body()}")
        logger.debug("3. Code: ${cloudEventResponse.code()}")
        logger.debug("4. Message: ${cloudEventResponse.message()}")
//        logger.debug("5. Headers: ${cloudEventResponse.headers()}")
//        logger.debug("6. Raw: ${cloudEventResponse.raw()}")
        logger.debug("7. Body(Size): ${cloudEventResponse.body()?.size} ")
        logger.debug("************* fetchCloudEvents FETCHED DONE *************\n")
    }
}
